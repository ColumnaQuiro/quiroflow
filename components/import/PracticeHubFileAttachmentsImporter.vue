...
<template>
  <div>
    ...
    <div v-else-if="stage === 'preview'" class="mt-4 space-y-4">
      <div class="rounded-lg border border-line bg-surface p-4">
        ...
        <dl class="mt-2 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
          ...
          <div><dt class="text-ink-muted2">Already imported</dt><dd class="font-medium text-ink-900">{{ skippedDuplicate }}</dd></div>
          <div><dt class="text-ink-muted2">No matching patient</dt><dd class="font-medium text-ink-900">{{ skippedUnmatched }}</dd></div>
        </dl>
      </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-line-divider">
            <tr v-for="(row, i) in toImport.slice(0, 10)" :key="i">
              ...
              <td class="px-3 py-2 text-ink-muted2">{{ row.file.file_type ?? 'N/A' }}</td>
              <td class="px-3 py-2 text-ink-muted2">{{ row.file.size_bytes ? `${(row.file.size_bytes / 1024).toFixed(0)} KB` : 'N/A' }}</td>
              <td class="px-3 py-2 text-ink-muted2">{{ row.file.created_at ? new Date(row.file.created_at).toLocaleDateString() : 'N/A' }}</td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-else-if="stage === 'importing'" class="mt-4 rounded-lg border border-line bg-surface p-8 text-center">
      <p class="text-sm text-ink-600">Importing… {{ importedCount }} / {{ toImport.length }}</p>
    </div>
