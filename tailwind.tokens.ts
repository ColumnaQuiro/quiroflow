// The design tokens, and nothing that has to be imported to use them.
//
// Split out of tailwind.config.ts so mobile/ can share them. mobile's own
// config used to spread the root config directly, which meant loading a file
// whose second line is `import plugin from 'tailwindcss/plugin'` -- resolved
// from the REPO ROOT, where Xcode Cloud never runs npm ci (it installs
// mobile/ only). Adding the `touch:` variant there therefore broke every
// mobile build with "Cannot find module 'tailwindcss/plugin'", and nothing in
// PR CI noticed, because CI does not build the mobile app.
//
// Anything needing a runtime import belongs in tailwind.config.ts, not here.
