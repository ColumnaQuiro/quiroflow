-- Online bookings didn't assign a room at all (room_id always null), so
-- practitioners and reception had no way to know where a self-booked
-- patient should go. Auto-pick any calendar_resource for the clinic that
-- has no overlapping appointment for the requested slot -- with only one
-- room configured (the onboarding default) this always resolves to that
-- room; clinics with several just get the first free one, ordered by name
-- for a stable, predictable assignment.
