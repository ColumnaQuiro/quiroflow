...
async function sendWhatsApp(
  ...
  // Cancelar reply options), so both mark the appointment 'pending' on the
  // way out -- that's what surfaces it as "unconfirmed" on the calendar
  // while we wait for a reply. Skip only if the patient already confirmed,
  // so a reminder sent after a confirmed reply doesn't reset them back to
  // unconfirmed. The same guard exists on the automation-rule send path in
  // runAutomationActions.ts.
  if (status === 'sent') {
