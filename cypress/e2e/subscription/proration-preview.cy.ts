...
describe('Subscription proration preview', () => {
  it('previews the prorated charge before switching plans, then confirms it', () => {
    seedActiveSubscriber().then(() => {
      ...
      cy.wait('@preview').its('request.body').should('deep.equal', { planId: 'pro', interval: 'monthly', extraProfessionals: 0 })
      ...
      cy.wait('@subscribe').its('request.body').should('deep.equal', { planId: 'pro', interval: 'monthly', extraProfessionals: 0 })
    })
  })
