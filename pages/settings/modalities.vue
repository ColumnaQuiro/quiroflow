...
<template>
  <div class="flex h-full flex-col">
    <PageHeader title="Modalities" />
    <div class="flex-1 overflow-y-auto">
      <div class="flex gap-8 p-6">
        <SettingsNav />
        <div class="min-w-0 max-w-[660px] flex-1">
          <p class="text-[13px] text-ink-muted2">Categorise practitioners, appointment types, and services by what's being provided (e.g. Chiropractic, Sports Massage, Osteopathy).</p>
            ...
            <table class="w-full text-[13px]">
              ...
              <tbody class="divide-y divide-line-row">
                ...
                <tr v-else-if="modalities.length === 0">
                  <td colspan="3" class="px-4 py-6 text-center text-ink-faint">No modalities yet.</td>
                </tr>
          ...
          <form class="mt-4 flex flex-wrap items-end gap-3 rounded-card border border-line bg-surface p-4 shadow-card" @submit.prevent="addModality">
            <div>
              ...
              <input v-model="name" type="text" required placeholder="Sports Massage" class="mt-1 h-8 rounded-ctl border border-line-control bg-surface px-3 text-[13px] text-ink-700 focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand/20" />
            </div>
            <UiBtn variant="primary" type="submit" :disabled="saving">{{ saving ? 'Adding…' : 'Add Modality' }}</UiBtn>
          </form>
