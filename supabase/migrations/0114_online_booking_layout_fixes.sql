-- Two Layout-tab settings (pages/settings/online-booking.vue) were dead:
-- 1. "Practitioner display order" sorted by tm.full_name either way, so
--    Default and Alphabetical produced identical output. Default now means
--    natural (created_at) order, distinct from Alphabetical.
-- 2. "Hide business logo" had nothing to hide -- the clinic logo was never
--    exposed to the public booking page at all. Now included so the widget
--    can render it (and honor the hide toggle).
